...
from ch08_testing.solution.support.logger import log
...
class DocumentManager:
    ...
    def retrieve(self, filename):
        d = None
        try:
            self.cursor.execute('SELECT * FROM {tbl} WHERE {column}="{value}"'.format(tbl='documents', column='filename', value=filename))
            doc = self.cursor.fetchone()
            if doc:
                d = Document(doc['filename'], doc['path'], doc['is_public'], doc['filesize'], doc['when_added'])
